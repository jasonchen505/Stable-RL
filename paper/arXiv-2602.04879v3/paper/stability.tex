...
The RL fine-tuning of LLMs is prone to training instability due to \textit{training-inference mismatch} (see \Cref{app:related_work_mismatch}). 
In this section, we conduct an empirical study to dissect this issue and verify the stability of our DPPO algorithm.
% In this section, we conduct an empirical study to dissect the training instability issue caused by training-inference mismatch (see Appendix \ref{app:related_work_mismatch}). 
To formalize our analysis, we denote the parameters being optimized as $\theta$ and the parameters used for data generation as $\theta'$. We aim to answer three fundamental research questions:
...
Our results also reveal an unexpected drawback of Truncated Importance Sampling (TIS), a common technique for reducing policy-gradient variance \citep{yao2025offpolicy, chen2025minimax}. In our experiments, TIS worsens training stability: as shown in \Cref{fig:sanity_test}, PG-TIS and MiniRL-TIS collapse prematurely and substantially underperform their untruncated counterparts. 
...
% We hypothesize that, like PPO ratio clipping, TIS disproportionately suppresses low-probability tokens, whose large ratios carry high-variance but important gradient signals. This variance reduction therefore introduces a harmful bias, suggesting that naive truncation can be as damaging as naive clipping.